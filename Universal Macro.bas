...
Attribute New_Invoice_Sheet.VB_ProcData.VB_Invoke_Func = "u\n14"
'
' New_Invoice_Sheet Macro
'
...
' Update Amount(£), Payment No., Previously Certified, Payment Bought Forward and Date
    ...
    Workbooks("Macro Book.xlsm").Worksheets("Info").Range("A22:A47").Copy
...
'    Dim InvoiceBook As Workbook
    ...
    Else
        ...
        If answer = vbYes Then
            Workbooks("Macro Book.xlsm").Worksheets("Info").Range("H4:L8").Copy
            ActiveWorkbook.ActiveSheet.Range("F63").PasteSpecial
        ElseIf answer = vbNo Then
            Workbooks("Macro Book.xlsm").Worksheets("Info").Range("B4:F8").Copy
            ActiveWorkbook.ActiveSheet.Range("F63").PasteSpecial
        End If
    End If
